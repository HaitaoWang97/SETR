...
class ASPPModule(nn.ModuleList):
    ...
    def __init__(self, dilations, in_channels, channels, norm_cfg,
                 act_cfg):
...
class AsppVit(nn.Module):
    ...
    def __init__(self, img_size=[192, 96, 48, 24], patch_size=[4, 4, 4, 4], in_chans=[256, 512, 1024, 2048],
        ...
        super(AsppVit, self).__init__(**kwargs)
        self.img_size = img_size
        self.patch_size = patch_size
        ...
        # self.num_patches = [self.patch_embed[i].num_patches for i in range(1, self.num_stages)]
    ...
    def forward(self, fp):
        outs = []
        x = fp[0]
        outs.append(self.aspp_module(x))
        for i in range(1, len(fp)):
            ...
            for j, blk in enumerate(self.blocks[i-1]):
                x = blk(x)
